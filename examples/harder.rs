...
use scr_axum_cranker_router::router_socket::RouterSocket;
use scr_axum_cranker_router::router_socket_filter::RouterSocketFilter;
...
async fn main() {
    ...
    let cranker_router = CrankerRouterBuilder::new()
        ...
        .with_router_socket_filter(Arc::new(MySocketFilter {}))
        .build();
    ...
    let custom_axum_wss_router = Router::new()
        .route(
            "/register",
            any(CrankerRouter::register_handler).layer(from_fn_with_state(
                cranker_router.state(),
        ...
        .route(
            "/deregister",
            any(CrankerRouter::de_register_handler).layer(from_fn_with_state(
                cranker_router.state(),
                CrankerRouter::de_reg_check,
            )),
        )
        .route(
            "/deregister/",
    ...
    let custom_axum_visit_portal_router = Router::new()
        ...
        .route(
            "/health/connectors",
            get(CrankerRouter::connector_info_handler),
        )
    ...
    println!(
        "trying to start harder server at - wss reg {}:{}, visit portal {}:{}",
        test_env.addr, test_env.reg_port, test_env.addr, test_env.visit_port
    );
    ...
    let _ = try_join!(
        axum::serve(reg_listener, reg_router)
            .with_graceful_shutdown(sig(cranker_router.clone()).into_future())
            .into_future(),
        axum::serve(visit_listener, visit_router)
            .with_graceful_shutdown(sig(cranker_router.clone()).into_future())
            .into_future(),
    );
}
...
impl ProxyListener for DemoProxyListener {
    fn on_before_proxy_to_target(
        ...
        info!(
            "[{}] on_before_proxy_to_target: header={:?}",
            id, request_headers_to_target
        );
        Ok(())
    }
    ...
    fn on_after_target_to_proxy_headers_received(
        ...
        info!(
            "[{}] on_after_target_to_proxy_headers_received: {:?} {:?}",
...
fn get_test_env() -> TestEnv {
    let addr = std::env::var("ACT_ADDR")
        .ok()
        .and_then(|s| IpAddr::from_str(&s).ok())
        .unwrap_or(IpAddr::V4(Ipv4Addr::from([127, 0, 0, 1])));
    let is_nop_logger = std::env::var("ACT_IS_NOP_LOGGER")
        .ok()
        .and_then(|s| bool::from_str(&s).ok())
        .unwrap_or(true);
    let log_level = std::env::var("ACT_LOG_LEVEL")
        .ok()
        .and_then(|s| LevelFilter::from_str(&s).ok())
        .unwrap_or(Debug);
    let reg_port = std::env::var("ACT_REG_PORT")
        .ok()
        .and_then(|s| u16::from_str(&s).ok())
        .unwrap_or(3000);
    let visit_port = std::env::var("ACT_VISIT_PORT")
        .ok()
        .and_then(|s| u16::from_str(&s).ok())
        .unwrap_or(3002);
    TestEnv {
...
struct MySocketFilter {}

impl RouterSocketFilter for MySocketFilter {
    fn should_use(
        &self,
        _: String,
        _: Method,
        ...
        _: HeaderMap,
        _: SocketAddr,
        _: CrankerRouterConfig,
        router_socket: Arc<dyn RouterSocket>,
    ) -> bool {
        router_socket.component_name().as_str().eq("bgm-archive-kt")
    }
    ...
    fn should_fallback_to_catch_all(&self) -> bool {
        true
    }
}
